...
export async function POST(req: NextRequest) {
  ...
  if (!leadId && !telefone) {
    return NextResponse.json({ error: "informe lead_id ou telefone" }, { status: 400 });
  }
  ...
  const fields = "id,nome,telefone,email,cliente_id,criado_em,ctwaclid,page_id,anuncio_source_id,plataforma,campanha,conjunto,anuncio";
  let lead: any = null;
  ...
  const eventTime = Math.floor(new Date(lead.criado_em).getTime() / 1000);
  const eventId = `leadsubmitted_${lead.id}_${eventTime}`;
  ...
  if (lead.email) userData.em = [sha256(String(lead.email).trim().toLowerCase())];
  ...
  if (auditError) {
    console.error("Falha ao registrar preview LeadSubmitted", auditError);
  ...
  return NextResponse.json({
    ...
    lead: {
      id: lead.id,
      nome: lead.nome,
      anuncio_source_id: lead.anuncio_source_id,
