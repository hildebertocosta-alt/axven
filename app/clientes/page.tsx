...
export default function ClientesPage() {
  ...
  return (
    <AppShell title="Clientes" subtitle="Central operacional dos clientes da Axven" activeLabel="Clientes">
      <div className="mx-auto max-w-[1500px]">
        <header className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.22em] text-[#caa45c]">Axven Executive OS</p>
            <h2 className="text-2xl font-semibold tracking-tight">Visão da carteira</h2>
            <p className="mt-2 text-sm text-zinc-500">Dados operacionais e financeiros dos clientes.</p>
          </div>
        ...
        <section className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#111316]">
          <div className="flex flex-col gap-3 border-b border-white/[0.06] p-4 md:flex-row md:items-center md:justify-between">
            <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar cliente ou nicho..." className="h-11 w-full rounded-xl border border-white/[0.08] bg-[#0c0e10] px-4 text-sm outline-none placeholder:text-zinc-700 focus:border-[#caa45c]/50 md:max-w-md" />
