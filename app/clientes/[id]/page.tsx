...
export default function Cliente360Page() {
  ...
  if (loading) {
    return <AppShell title="Cliente 360º" subtitle="Central operacional do cliente" activeLabel="Clientes"><div className="p-10 text-zinc-500">Carregando cliente...</div></AppShell>;
  }

  if (erro || !cliente) {
    return <AppShell title="Cliente 360º" subtitle="Central operacional do cliente" activeLabel="Clientes"><div className="p-10"><Link href="/clientes" className="text-[#d7b66f]">← Voltar para clientes</Link><p className="mt-6 text-rose-400">{erro || "Cliente não encontrado"}</p></div></AppShell>;
  }
  ...
  const implemented = ["Visão geral", "Campanhas", "Leads", "Criativos"];
        ...
        {activeTab === "Campanhas" && <div className="space-y-5">
          <section className="grid gap-5 xl:grid-cols-[1fr_.8fr]">
            <div className="rounded-2xl border border-white/[0.07] bg-[#111316] p-6">
              ...
              {!metaLoading && !metaErro && <div className="mt-6 flex flex-col gap-3 md:flex-row"><select value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)} className="h-11 min-w-0 flex-1 rounded-xl border border-white/[0.08] bg-[#0c0e10] px-4 text-sm text-zinc-300 outline-none"><option value="">Selecione a conta de anúncios...</option>{metaContas.map((conta) => <option key={conta.account_id} value={conta.account_id}>{conta.name || conta.business_name || "Conta Meta"} · {conta.account_id}</option>)}</select><button onClick={vincularContaMeta} disabled={!selectedAccount || savingMeta} className="h-11 rounded-xl bg-[#caa45c] px-5 text-sm font-semibold text-[#111214] disabled:opacity-40">{savingMeta ? "Vinculando..." : cliente.meta_account_id ? "Alterar vínculo" : "Vincular conta"}</button></div>}
            </div>
            <div className="rounded-2xl border border-white/[0.07] bg-[#111316] p-6"><p className="text-xs uppercase tracking-[0.15em] text-zinc-600">Status da integração</p><h3 className="mt-2 text-lg font-semibold">Campanhas</h3>{cliente.meta_account_id ? <div className="mt-6"><span className="inline-flex rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">● Conta vinculada</span><p className="mt-4 text-sm font-medium text-zinc-300">{contaVinculada?.name || contaVinculada?.business_name || "Conta Meta"}</p><p className="mt-1 text-xs text-zinc-600">ID {cliente.meta_account_id}</p><button onClick={() => carregarCampanhas(true)} className="mt-5 text-xs font-medium text-[#d7b66f]">↻ Atualizar dados Meta</button></div> : <div className="mt-6 rounded-xl border border-amber-500/15 bg-amber-500/[0.06] p-4 text-sm text-amber-200/80">Nenhuma conta Meta vinculada a este cliente.</div>}</div>
          </section>

          {cliente.meta_account_id && campanhasLoading && <div className="rounded-2xl border border-white/[0.07] bg-[#111316] p-8 text-sm text-zinc-500">Carregando campanhas e métricas reais da Meta...</div>}
          {cliente.meta_account_id && campanhasErro && <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-5 text-sm text-rose-300">{campanhasErro}</div>}
          {cliente.meta_account_id && !campanhasLoading && !campanhasErro && campanhasData && <>
            ...
            <section className="overflow-hidden rounded-2xl border border-white/[0.07] bg-[#111316]"><div className="border-b border-white/[0.07] px-6 py-5"><h3 className="text-lg font-semibold text-zinc-100">Campanhas</h3><p className="mt-1 text-xs text-zinc-600">{campanhasData.periodo}</p></div><div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-[#0c0e10] text-[10px] uppercase tracking-[0.12em] text-zinc-600"><tr><th className="px-6 py-3">Campanha</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Investimento</th><th className="px-4 py-3">Leads</th><th className="px-4 py-3">CPL</th><th className="px-4 py-3">Mensagens</th><th className="px-4 py-3">Cliques</th><th className="px-4 py-3">CTR</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{campanhasData.campanhas.map((c) => <tr key={c.id} className="text-zinc-300"><td className="px-6 py-4"><p className="font-medium text-zinc-200">{c.name}</p></td><td className="px-4 py-4">{text(c.effective_status)}</td><td className="px-4 py-4">{money(c.spend, 2)}</td><td className="px-4 py-4">{number(c.leads)}</td><td className="px-4 py-4">{c.cpl != null ? money(c.cpl, 2) : "—"}</td><td className="px-4 py-4">{number(c.mensagens)}</td><td className="px-4 py-4">{number(c.clicks)}</td><td className="px-4 py-4">{c.ctr.toFixed(2)}%</td></tr>)}</tbody></table></div></section>
          </>}
        </div>}
