...
export default function LeadForm() {
  ...
  useEffect(() => {
    ...
    trackingKeys.forEach((key) => { const value = params.get(key); if (value) captured[key] = value; });
    setTracking(captured);
  }, []);

  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (sending) return; setSending(true); setSent(null);
    const data = new FormData(event.currentTarget); const payload = Object.fromEntries(data.entries());
    try {
      const response = await fetch("/api/aquisicao/clinicas", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, ...tracking, origin_url: window.location.href }) });
      if (!response.ok) throw new Error("request_failed");
      const result = await response.json(); setSent(result.qualified ? "qualified" : "unqualified");
    } catch { setSent("error"); } finally { setSending(false); }
  }

  if (sent === "qualified") return <Result title="Sua clínica tem perfil para avançar." text="Com base nas informações enviadas, o próximo passo é uma Análise de Crescimento Axven. O calendário será conectado aqui na etapa de agenda." />;
  ...
  return <>
    {sent === "error" && <div className="mt-8 rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-center text-sm text-red-200">Não foi possível enviar agora. Tente novamente em instantes.</div>}
