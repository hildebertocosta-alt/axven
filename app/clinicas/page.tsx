    ...
    <main className="min-h-screen bg-[#0B0B0F] text-[#F5F5F7]">
      ...
      <section className="border-y border-[#2A2A32] bg-[#111117]">
        <div className="mx-auto max-w-6xl px-6 py-20">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-2">{["Quais anúncios trouxeram as melhores oportunidades?","Quantos leads realmente agendaram?","Quantos compareceram?","Quantos compraram e quanto geraram em vendas?"].map(x=><div key={x} className="rounded-2xl border border-[#2A2A32] bg-[#17171D] p-5">{x}</div>)}</div>
        </div>
      </section>

      <section className="mx-auto max-w-6xl px-6 py-24">
        <div className="grid items-center gap-12 lg:grid-cols-[.9fr_1.1fr]">
          <div className="relative min-h-[460px] overflow-hidden rounded-[28px] border border-[#2A2A32] bg-[#111117] shadow-2xl">
            <img src="https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?auto=format&fit=crop&fm=jpg&q=84&w=1600" alt="Profissional de estética atendendo paciente em clínica premium" className="absolute inset-0 h-full w-full object-cover" />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0B0B0F]/90 via-transparent to-[#0B0B0F]/10" />
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8">
              <div className="inline-flex rounded-full border border-white/10 bg-black/45 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.14em] text-[#FF8A64] backdrop-blur-md">Contexto real de operação</div>
              <p className="mt-4 max-w-md text-lg font-medium leading-7 text-white">A experiência da paciente acontece na clínica. A Axven organiza o caminho que traz a oportunidade até ela.</p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-10 grid gap-4 sm:grid-cols-2">{steps.map(([n,t,d])=><article key={n} className="rounded-2xl border border-[#2A2A32] bg-[#111117] p-5"><span className="text-sm font-semibold text-[#FF5A3C]">{n}</span><h3 className="mt-3 text-lg font-semibold">{t}</h3><p className="mt-2 text-sm leading-6 text-[#9B9BA4]">{d}</p></article>)}</div>
            <div className="mt-6 rounded-2xl border border-[#2A2A32] p-5 text-center text-sm text-[#9B9BA4]">Anúncio → Lead → Qualificação → Agendamento → Venda → Receita → Dados</div>
          </div>
        </div>
      </section>
