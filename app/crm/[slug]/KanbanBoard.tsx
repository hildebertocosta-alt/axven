...
import { atualizarEtapaLead } from "./actions";
...
function LeadCard({ lead, dragging = false, onTogglePausa }: { lead: LeadRow; dragging?: boolean; onTogglePausa?: (lead: LeadRow) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`cursor-grab rounded-2xl border border-white/10 bg-zinc-900/80 p-4 text-sm shadow-sm transition active:cursor-grabbing ${dragging ? "rotate-2 shadow-lg shadow-black/40" : ""}`}>
      ...
      {lead.origem ? <span className="mt-3 inline-flex rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[11px] text-zinc-400">{lead.origem}</span> : null}
      {lead.pausado_ia ? <span className="mt-2 inline-flex rounded-full border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-[11px] font-medium text-violet-200">🙋 Você assumiu essa conversa</span> : null}
      {onTogglePausa ? (
        <div className="mt-3 flex items-center gap-2">
          <button
            onPointerDown={(event) => event.stopPropagation()}
            onClick={(event) => { event.stopPropagation(); onTogglePausa(lead); }}
            className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-[11px] transition ${lead.pausado_ia ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-200 hover:bg-emerald-500/20" : "border-violet-500/30 bg-violet-500/10 text-violet-200 hover:bg-violet-500/20"}`}
          >
...
function KanbanColumn({ column, leads, onTogglePausa }: { column: Column; leads: LeadRow[]; onTogglePausa: (lead: LeadRow) => void }) {
  ...
  return (
    <div className="flex min-w-[280px] flex-1 flex-col">
      ...
      <div ref={setNodeRef} className={`flex min-h-[200px] flex-1 flex-col gap-3 rounded-3xl border p-3 transition ${column.accent} ${isOver ? "ring-2 ring-violet-500/40" : ""}`}>
        <SortableContext items={leads.map((lead) => lead.id)} strategy={verticalListSortingStrategy}>
          {leads.map((lead) => <LeadCard key={lead.id} lead={lead} onTogglePausa={onTogglePausa} />)}
        </SortableContext>
...
export function KanbanBoard({ clienteSlug, initialLeads }: { clienteSlug: string; initialLeads: LeadRow[] }) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    const targetColumn = COLUMNS.find((col) => col.key === over.id)?.key ?? leads.find((item) => item.id === over.id)?.etapa;
    if (!targetColumn || targetColumn === draggedLead.etapa) return;

    const previousLeads = leads;
    const updatedAt = new Date().toISOString();
    setLeads((prev) => prev.map((item) => item.id === draggedLead.id ? { ...item, etapa: targetColumn, atualizado_em: updatedAt } : item));

    const result = await atualizarEtapaLead({ slug: clienteSlug, leadId: draggedLead.id, etapa: targetColumn });
    if (!result.ok) {
      setLeads(previousLeads);
      console.error("Falha ao atualizar etapa do lead:", result.error);
    }
  }
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onDragCancel={() => setActiveLead(null)}>
      <div className="flex gap-4 overflow-x-auto pb-2">
        {COLUMNS.map((column) => <KanbanColumn key={column.key} column={column} leads={leads.filter((lead) => lead.etapa === column.key)} onTogglePausa={handleTogglePausa} />)}
      </div>
